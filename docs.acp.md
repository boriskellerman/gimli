...
This document describes how the Gimli ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`gimli acp` exposes an ACP agent over stdio and forwards prompts to a running
Gimli Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Gimli Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `gimli acp` over stdio.

Example config:

```bash
gimli config set gateway.remote.url wss://gateway-host:18789
gimli config set gateway.remote.token <token>
```

Example run:

```bash
gimli acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
gimli acp --session agent:main:main
gimli acp --session agent:design:main
gimli acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Gimli ACP” to start a thread.

## Execution Model

- ACP client spawns `gimli acp` and speaks ACP messages over stdio.
...
```bash
gimli acp --session agent:main:main
gimli acp --session-label "support inbox"
gimli acp --reset-session
```
...
`gimli acp` resolves the Gateway URL and auth from CLI flags or config:
