...
const compiler = env.GIMLI_TS_COMPILER === "tsc" ? "tsc" : "tsgo";
...
const shouldBuild = () => {
  if (env.GIMLI_FORCE_BUILD === "1") return true;
...
const logRunner = (message) => {
  if (env.GIMLI_RUNNER_LOG === "0") return;
  process.stderr.write(`[gimli] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["gimli.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
