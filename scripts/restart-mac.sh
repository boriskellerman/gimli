#!/usr/bin/env bash
# Reset Gimli like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${GIMLI_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Gimli.app/Contents/MacOS/Gimli"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Gimli"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Gimli"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Gimli"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/bot.molt.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/gimli-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${GIMLI_RESTART_LOG:-/tmp/gimli-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${GIMLI_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.gimli/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  GIMLI_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node gimli.mjs daemon install --force --runtime node"
      log "  node gimli.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.gimli/disable-launchagent"
      log ""
...
log "==> Killing existing Gimli instances"
kill_all_gimli
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Gimli"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Gimli.app" ]]; then
    APP_BUNDLE="/Applications/Gimli.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Gimli.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Gimli.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Gimli.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set GIMLI_APP_BUNDLE to your installed Gimli.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node gimli.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node gimli.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".gimli", "gimli.json"), "utf8");
