...
#
# Suggested cron: */30 * * * * /home/admin/gimli/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Gimli notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., gimli-alerts)
...
STATE_FILE="$HOME/.gimli/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Gimli if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use gimli
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Gimli to $NOTIFY_PHONE..."
            gimli send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Gimli is down. Run: ssh l36 '~/gimli/scripts/mobile-reauth.sh'" "urgent"
    exit 1
