...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/gimli/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Gimli Re-Auth" -c "After getting key, run: ssh $SERVER '~/gimli/scripts/mobile-reauth.sh'" --id gimli-auth
        ;;
    *)
