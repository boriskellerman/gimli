...
# Build and bundle Gimli into a minimal .app we can open.
# Outputs to dist/Gimli.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Gimli.app"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/gimli/gimli/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Gimli/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :GimliBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :GimliGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Gimli"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Gimli"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Gimli"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Gimli" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Gimli/Resources/Gimli.icns" "$APP_ROOT/Contents/Resources/Gimli.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Gimli/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying GimliKit resources"
GIMLIKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/GimliKit_GimliKit.bundle"
if [ -d "$GIMLIKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/GimliKit_GimliKit.bundle"
  cp -R "$GIMLIKIT_BUNDLE" "$APP_ROOT/Contents/Resources/GimliKit_GimliKit.bundle"
else
  echo "WARN: GimliKit resource bundle not found at $GIMLIKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Gimli"
killall -q Gimli 2>/dev/null || true
