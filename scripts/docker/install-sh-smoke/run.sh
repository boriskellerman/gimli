...
SMOKE_PREVIOUS_VERSION="${GIMLI_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${GIMLI_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="gimli"
if [[ -z "${GIMLI_INSTALL_PACKAGE:-}" && "$INSTALL_URL" == *"gimli.bot"* ]]; then
  DEFAULT_PACKAGE="gimli"
fi
PACKAGE_NAME="${GIMLI_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
if [[ "$PACKAGE_NAME" == "gimli" ]]; then
  ALT_PACKAGE_NAME="gimli"
else
  ALT_PACKAGE_NAME="gimli"
fi
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (GIMLI_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${GIMLI_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "$GIMLI_INSTALL_LATEST_OUT"
fi
