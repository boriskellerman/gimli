...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/gimli-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/gimli-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/gimli-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/gimli"
    config_path="$HOME/.gimli/gimli.json"
    sessions_dir="$HOME/.gimli/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.gimli/gimli.json" <<'"'"'JSON'"'"'
{
