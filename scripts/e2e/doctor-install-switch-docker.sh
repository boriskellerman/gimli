...
IMAGE_NAME="gimli-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/gimli-bin:$PATH"
  mkdir -p /tmp/gimli-bin

  cat > /tmp/gimli-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/gimli-bin/systemctl

  cat > /tmp/gimli-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/gimli-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/gimli"
  npm_entry="/tmp/npm-prefix/lib/node_modules/gimli/gimli.mjs"
  git_entry="/app/gimli.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/gimli-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/gimli-gateway.service"
    if [ ! -f "$unit_path" ]; then
