...
IMAGE_NAME="gimli-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="gimli-net-e2e-$$"
GW_NAME="gimli-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "GIMLI_GATEWAY_TOKEN=$TOKEN" \
	  -e "GIMLI_SKIP_CHANNELS=1" \
	  -e "GIMLI_SKIP_GMAIL_WATCHER=1" \
	  -e "GIMLI_SKIP_CRON=1" \
	  -e "GIMLI_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
