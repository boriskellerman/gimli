...
ZIP=${1:?"Usage: $0 Gimli-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/gimli/gimli/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Gimli-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/gimli/gimli/releases/download/v${VERSION}/"}
