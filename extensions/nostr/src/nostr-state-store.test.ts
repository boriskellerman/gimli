...
import type { PluginRuntime } from "gimli/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.GIMLI_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "gimli-nostr-"));
  process.env.GIMLI_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.GIMLI_STATE_DIR?.trim();
        if (override) return override;
        return path.join(homedir(), ".gimli");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) delete process.env.GIMLI_STATE_DIR;
    else process.env.GIMLI_STATE_DIR = previous;
