...
export function listBlueBubblesAccountIds(cfg: GimliConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultBlueBubblesAccountId(cfg: GimliConfig): string {
...
export function listEnabledBlueBubblesAccounts(cfg: GimliConfig): ResolvedBlueBubblesAccount[] {
