...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["gimliPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["gimli", "pollId"]) ??
    readNestedString(value, ["gimli", "poll", "id"]) ??
    readNestedString(value, ["data", "gimliPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "gimli", "pollId"]);
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "gimli poll vote",
          displayText: "Vote recorded",
          value: { gimliPollId: pollId },
        },
      },
    ],
  };
