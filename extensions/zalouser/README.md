...
Gimli extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
gimli channels login --channel zalouser
...
```bash
gimli message send --channel zalouser --target <threadId> --message "Hello from Gimli!"
```
...
```bash
gimli channels login --channel zalouser              # Login via QR
gimli channels login --channel zalouser --account work
gimli channels status --probe
gimli channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
gimli directory self --channel zalouser
gimli directory peers list --channel zalouser --query "name"
gimli directory groups list --channel zalouser --query "work"
gimli directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
gimli message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
gimli message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `gimli channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
gimli channels login --channel zalouser --account work
gimli message send --channel zalouser --account work --target <id> --message "Hello"
