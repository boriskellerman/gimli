...
function resolveGimliRoot(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.GIMLI_ROOT?.trim() || process.env.GIMLI_ROOT?.trim();
  if (override) {
  ...
  throw new Error(
    "Unable to resolve core root. Set GIMLI_ROOT (or legacy GIMLI_ROOT) to the package root.",
  );
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveGimliRoot();
