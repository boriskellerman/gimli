/**
 * Twitch channel plugin for Gimli.
 *
...
import { buildChannelConfigSchema } from "gimli/plugin-sdk";
 ...
 * for Gimli. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: GimliConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: GimliConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: GimliConfig): boolean => {
