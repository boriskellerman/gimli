...
// NOTE: This extension is intended to be bundled with Gimli.
// When running from source (tests/dev), Gimli internals live under src/.
...
export function createLlmTaskTool(api: GimliPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via gimli.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "gimli-llm-task-"));
