...
`gimli.invoke --each`) without adding new Gimli code per workflow.
...
This extension depends on Gimli internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Gimli extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.gimli/extensions` as a standalone plugin directory.
