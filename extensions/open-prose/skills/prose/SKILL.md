...
metadata: {"gimli":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
- **Task tool** in the upstream spec == Gimli `sessions_spawn`
- **File I/O** == Gimli `read`/`write`
- **Remote fetch** == Gimli `web_fetch` (or `exec` with curl when POST is required)
