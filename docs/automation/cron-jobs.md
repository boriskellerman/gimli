...
- Jobs persist under `~/.gimli/cron/` so restarts don’t lose schedules.
...
## Storage & history
- Job store: `~/.gimli/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.gimli/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
gimli cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
gimli cron edit <jobId> --agent ops
gimli cron edit <jobId> --clear-agent
```
```
...
```bash
gimli system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`gimli system event`](/cli/system).

## Troubleshooting

### “Nothing runs”
- Check cron is enabled: `cron.enabled` and `GIMLI_SKIP_CRON`.
