...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `gimli sandbox explain`.
...
Legacy `agent.*` configs are migrated by `gimli doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=gimli.sandbox=1"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${GIMLI_STATE_DIR:-$HOME/.gimli}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
