...
Matrix is an open, decentralized messaging protocol. Gimli connects as a Matrix **user**
...
```bash
gimli plugins install @gimli/matrix
```

Local checkout (when running from a git repo):

```bash
gimli plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `gimli plugins install @gimli/matrix`
   - From a local checkout: `gimli plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Gimli calls the same
     login endpoint, stores the access token in `~/.gimli/credentials/matrix/credentials.json`,
...
- On first connection, Gimli requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Gimli logs a warning.
...
`~/.gimli/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `gimli pairing list matrix`
  - `gimli pairing approve matrix <CODE>`
...
- On startup, Gimli resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
