...
read_when:
  - You want Gimli to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Gimli to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`gimli onboard`) and `gimli channels add` list optional channel plugins.
...
```bash
gimli plugins install @gimli/nostr
```

Use a local checkout (dev workflows):

```bash
gimli plugins install --link <path-to-gimli>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "gimli@example.com",
        "lud16": "gimli@example.com"
      }
    }
  }
}
