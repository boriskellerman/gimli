...
```bash
gimli plugins install @gimli/msteams
```

Local checkout (when running from a git repo):
```bash
gimli plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
Gimli will offer the local install path automatically.
...
3) Configure Gimli with those credentials.
...
## Goals
- Talk to Gimli via Teams DMs, group chats, or channels.
...
- On startup, Gimli resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.gimli/gimli.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Gimli, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field | Value |
   |-------|-------|
   | **Bot handle** | Your bot name, e.g., `gimli-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `gimli plugins install @gimli/msteams`
   - From a local checkout: `gimli plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Gimli in Teams", "full": "Gimli in Teams" },
...
Gimli handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Gimli only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/GimliShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)
Gimli sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `gimli message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.gimli/msteams-polls.json`.
...
**CLI:**
```bash
gimli message send --channel msteams \
...
gimli message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
gimli message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
gimli message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
gimli message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
