...
Tlon is a decentralized messenger built on Urbit. Gimli connects to your Urbit ship and can
...
```bash
gimli plugins install @gimli/tlon
```

Local checkout (when running from a git repo):

```bash
gimli plugins install ./extensions/tlon
```
...
Use these with `gimli message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Gimli replies in-thread.
