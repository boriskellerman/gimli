...
read_when:
  - Setting up Twitch chat integration for Gimli
---
# Twitch (plugin)

Twitch chat support via IRC connection. Gimli connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
gimli plugins install @gimli/twitch
```

Local checkout (when running from a git repo):

```bash
gimli plugins install ./extensions/twitch
```
...
4) Configure the token:
   - Env: `GIMLI_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "gimli",              // Bot's Twitch account
      accessToken: "oauth:abc123...",    // OAuth Access Token (or use GIMLI_TWITCH_ACCESS_TOKEN env var)
...
```bash
GIMLI_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
