...
```bash
gimli plugins install @gimli/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
gimli plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Gimli" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `gimli pairing list nextcloud-talk`
  - `gimli pairing approve nextcloud-talk <CODE>`
