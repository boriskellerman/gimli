...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Gimli
---

# Node + tsx "__name is not a function" crash

## Summary
Running Gimli via Node with `tsx` fails at startup with:

```
[gimli] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `gimli status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch gimli.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node gimli.mjs status` works on Node 25.
