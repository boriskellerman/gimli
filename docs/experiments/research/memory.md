---
summary: "Research notes: offline memory system for Gimli workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/gimli) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Gimli integration
  - Adding offline recall + reflection (retain/recall/reflect)
---

# Workspace Memory v2 (offline): research notes

Target: Gimli-style workspace (`agents.defaults.workspace`, default `~/gimli`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/gimli` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Gimli to see every session.
...
Recommendation: **deep integration in Gimli**, but keep a separable core library.

### Why integrate into Gimli?
- Gimli already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `gimli memory recall "…" --k 25 --since 30d`
  - `gimli memory reflect --since 7d`
