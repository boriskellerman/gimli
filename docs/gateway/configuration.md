---
summary: "All configuration options for ~/.gimli/gimli.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

Gimli reads an optional **JSON5** config from `~/.gimli/gimli.json` (comments + trailing commas allowed).

If the file is missing, Gimli uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/gimli`). You usually only need a config to:
...
Gimli only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `gimli doctor`, `gimli logs`, `gimli health`, `gimli status`, `gimli service`, `gimli help`).
- Run `gimli doctor` to see the exact issues.
- Run `gimli doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `gimli config set`. Keep a backup of `~/.gimli/gimli.json`.
...
```bash
gimli gateway call config.get --params '{}' # capture payload.hash
gimli gateway call config.apply --params '{
...
```bash
gimli gateway call config.get --params '{}' # capture payload.hash
gimli gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/gimli" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@gimli", "reisponde"] }
      }
    ]
  },
  channels: {
...
{ "$include": "/etc/gimli/base.json5" } // absolute
...
```json5
// ~/.gimli/clients/mueller/agents.json5
[
...
```json5
// ~/.gimli/clients/mueller/broadcast.json5
{
...
Gimli reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Gimli runs your login shell and imports only the missing expected keys (never overrides).
...
Env var equivalent:
- `GIMLI_LOAD_SHELL_ENV=1`
- `GIMLI_SHELL_ENV_TIMEOUT_MS=15000`
...
Gimli stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.gimli/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.gimli/credentials/oauth.json` (or `$GIMLI_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.gimli/agent/*` (migrated by `gimli doctor` into `~/.gimli/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `GIMLI_OAUTH_DIR`
- Agent dir (default agent root override): `GIMLI_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Gimli imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Gimli derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/gimli/gimli-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/gimli/gimli.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.gimli/credentials/whatsapp/biz
          // authDir: "~/.gimli/credentials/whatsapp/biz",
        }
      }
    }
  }
...
- The legacy single-account Baileys auth dir is migrated by `gimli doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@gimli", "gimli", "gimli"] } }
    ]
  }
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@gimli"]
        }
      }
    ]
  }
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/gimli-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.gimli/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/gimli-home" },
      { id: "work", workspace: "~/gimli-work" }
    ]
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `gimli.json`).
...
Gimli starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["gimli-dm"]          // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {               // guild id (preferred) or slug
          slug: "friends-of-gimli",
...
Gimli starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `gimli plugins install @gimli/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Gimli starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Gimli spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Gimli
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/gimli" } }
}
```
...
When a file exceeds this limit, Gimli logs a warning and injects a truncated
...
message envelopes). If unset, Gimli uses the host timezone at runtime.
...
`[gimli]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[gimli]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[gimli]"`, Gimli will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Gimli generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.gimli/settings/tts.json",
      elevenlabs: {
...
`voiceAliases` lets Talk directives use friendly names (e.g. `"voice":"Gimli"`).
...
Gimli also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Gimli currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `gimli configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:gimli`: all built-in Gimli tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.gimli/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.gimli/sandboxes",
        docker: {
          image: "gimli-sandbox:bookworm-slim",
          containerPrefix: "gimli-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
        ...
        browser: {
          enabled: false,
          image: "gimli-sandbox-browser:bookworm-slim",
          containerPrefix: "gimli-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Gimli uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.gimli/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Gimli config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Gimli writes/merges a `models.json` into
`~/.gimli/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Gimli uses
...
- Shortcut: `gimli onboard --auth-choice opencode-zen`.
...
Shortcut: `gimli onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `gimli onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `gimli onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `gimli onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `gimli onboard --auth-choice minimax-api`.
...
- Override the agent config root with `GIMLI_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.gimli/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.gimli/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.gimli/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Gimli stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.gimli/skills` (workspace skills
...
from `~/.gimli/extensions`, `<workspace>/.gimli/extensions`, plus any
...
### `browser` (gimli-managed browser)

Gimli can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for gimli and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (Gimli.app menubar, or `gimli gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      gimli: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/gimli" }
...
- When a connection comes from one of these IPs, Gimli uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `gimli gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `GIMLI_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `GIMLI_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Gimli
...
macOS app behavior:
- Gimli.app watches `~/.gimli/gimli.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.gimli/gimli.json` (or `GIMLI_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.gimli/gimli.json` (or `GIMLI_CONFIG_PATH`)
...
- `GIMLI_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `gimli --dev …` → uses `~/.gimli-dev` + shifts ports from base `19001`
- `gimli --profile <name> …` → uses `~/.gimli-<name>` (port via config/env/flags)
...
Example:
```bash
GIMLI_CONFIG_PATH=~/.gimli/a.json \
GIMLI_STATE_DIR=~/.gimli-a \
gimli gateway --port 19001
```
...
Gmail helper config (used by `gimli webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "gimli@gmail.com",
...
- Set `GIMLI_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Gimli defaults `serve.path` to `/` so
...
Default root: `~/gimli/canvas`  
Default port: `18793` (chosen to avoid the gimli browser CDP port `18792`)  
The server listens on the **gateway bind host** (LAN or Tailnet) so nodes can reach it.

The server:
- serves files under `canvasHost.root`
- injects a tiny live-reload client into served HTML
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__gimli/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__gimli__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.gimli/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.gimli/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.gimli/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
Controls LAN mDNS discovery broadcasts (`_gimli-gw._tcp`).
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_gimli-bridge._tcp` under `~/.gimli/dns/` using the standard discovery domain `gimli.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `gimli.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `gimli.internal` via that server
