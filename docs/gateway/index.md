...
- Replaces the legacy `gateway` command. CLI entry point: `gimli gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)
```bash
gimli gateway --port 18789
...
pnpm gateway:watch
```
- Config hot reload watches `~/.gimli/gimli.json` (or `GIMLI_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__gimli__/canvas/` from `~/gimli/canvas`. Disable with `canvasHost.enabled=false` or `GIMLI_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `GIMLI_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `GIMLI_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.gimli.*` may still exist)
- Linux: `gimli-gateway-<profile>.service`
- Windows: `Gimli Gateway (<profile>)`

Install metadata is embedded in the service config:
- `GIMLI_SERVICE_MARKER=gimli`
- `GIMLI_SERVICE_KIND=gateway`
- `GIMLI_SERVICE_VERSION=<version>`
...
```bash
gimli --dev setup
gimli --dev gateway --allow-unconfigured
...
- `GIMLI_STATE_DIR=~/.gimli-dev`
- `GIMLI_CONFIG_PATH=~/.gimli-dev/gimli.json`
- `GIMLI_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/gimli-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):
- Base port = `gateway.port` (or `GIMLI_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `GIMLI_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:
- unique `gateway.port`
- unique `GIMLI_CONFIG_PATH`
- unique `GIMLI_STATE_DIR`
- unique `agents.defaults.workspace`
- separate WhatsApp numbers (if using WA)

Service install per profile:
```bash
gimli --profile main gateway install
gimli --profile rescue gateway install
```

Example:
```bash
GIMLI_CONFIG_PATH=~/.gimli/a.json GIMLI_STATE_DIR=~/.gimli-a gimli gateway --port 19001
GIMLI_CONFIG_PATH=~/.gimli/b.json GIMLI_STATE_DIR=~/.gimli-b gimli gateway --port 19002
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `gimli health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `gimli`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `gimli gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.gimli.*` is cleaned up).
  - `gimli doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Gimli profile services.
  ...
  - Cleanup: `gimli gateway uninstall` (current service) and `gimli doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `gimli gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- Gimli.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.gimli.*` labels still unload cleanly).
- To stop it cleanly, use `gimli gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `gimli gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `gimli gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)
Gimli installs a **systemd user service** by default on Linux/WSL2. We
...
`gimli gateway install` writes the user unit. `gimli doctor` audits the
...
Restart=always
RestartSec=5
Environment=GIMLI_GATEWAY_TOKEN=
...
Create `/etc/systemd/system/gimli-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now gimli-gateway[-<profile>].service
```
...
## CLI helpers
- `gimli gateway health|status` — request health/status over the Gateway WS.
- `gimli message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `gimli agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `gimli gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `gimli gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `gimli gateway` and the legacy TCP control port.
