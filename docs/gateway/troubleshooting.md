---
summary: "Quick troubleshooting guide for common Gimli failures"
read_when:
  - Investigating runtime issues or failures
---
# Troubleshooting 🔧

When Gimli misbehaves, here's how to fix it.
...
|---|---|---|
| `gimli status` | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview |
| `gimli status --all` | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail | When you need to share a debug report |
| `gimli status --deep` | Runs gateway health checks (incl. provider probes; requires reachable gateway) | When “configured” doesn’t mean “working” |
| `gimli gateway probe` | Gateway discovery + reachability (local + remote targets) | When you suspect you’re probing the wrong gateway |
| `gimli channels status --probe` | Asks the running gateway for channel status (and optionally probes) | When gateway is reachable but channels misbehave |
| `gimli gateway status` | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error | When the service “looks loaded” but nothing runs |
| `gimli logs --follow` | Live logs (best signal for runtime issues) | When you need the actual failure reason |

**Sharing output:** prefer `gimli status --all` (it redacts tokens). If you paste `gimli status`, consider setting `GIMLI_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  gimli models auth setup-token --provider anthropic
  ```
...
**Logs:**
- Preferred: `gimli logs --follow`
- File logs (always): `/tmp/gimli/gimli-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$GIMLI_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u gimli-gateway[-<profile>].service -n 200 --no-pager`
...
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  gimli config set gateway.mode remote
  gimli config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `gimli setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.gimli/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `gimli doctor`
...
- then run `gimli sandbox recreate --agent <id>` (or `--all`)
...
**Check:**
- `gateway.mode` must be `local` for `gimli gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `gimli gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `gimli gateway status` and `gimli doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `GIMLI_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `gimli gateway status` shows a config mismatch**
...
- Fix: rerun `gimli gateway install --force` from the same `--profile` / `GIMLI_STATE_DIR` you want the service to use.

**If `gimli gateway status` reports service config issues**
- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `gimli doctor` to update it (or `gimli gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `GIMLI_GATEWAY_TOKEN`) and restart the service.

**If `gimli gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/gimli` on disk.
...
Symptoms: `pwd` or file tools show `~/.gimli/sandboxes/...` even though you
...
Gimli intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `gimli models list` or
  `gimli models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${GIMLI_CONFIG_PATH:-$HOME/.gimli/gimli.json}"
```
...
tail -f "$(ls -t /tmp/gimli/gimli-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
gimli logs --follow | grep "pairing request"
```
...
- ❌ `@gimli` + image
- ✅ `@gimli check this` + image

### Session Not Resuming

**Check 1:** Is the session file there?
```bash
ls -la ~/.gimli/agents/<agentId>/sessions/
```
...
gimli logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
gimli channels logout
trash "${GIMLI_STATE_DIR:-$HOME/.gimli}/credentials" # if logout can't cleanly remove everything
gimli channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/gimli/gimli-*.log | head -1)" | tail -20
```

### High Memory Usage

Gimli keeps conversation history in memory.
...
Gimli now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
Notes:
- `gimli doctor` reports every invalid entry.
- `gimli doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `gimli logs`, `gimli health`, `gimli status`, `gimli gateway status`, and `gimli gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/gimli/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `gimli models status` (CLI).
...
```bash
curl -fsSL https://gimli.bot/install.sh | bash -s -- --install-method git --no-onboard
```
...
5) Run `gimli channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. Gimli scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/gimli/gimli/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
gimli gateway status
gimli gateway stop
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.gimli.* still works)
```
...
Ensure the global `gimli` CLI is installed and matches the app version:
```bash
gimli --version
npm install -g gimli@<version>
```
...
#   ${GIMLI_CONFIG_PATH:-$HOME/.gimli/gimli.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
gimli gateway --verbose
gimli channels login --verbose
```
...
| Log | Location |
|-----|----------|
| Gateway file logs (structured) | `/tmp/gimli/gimli-YYYY-MM-DD.log` (or `logging.file`) |
| Gateway service logs (supervisor) | macOS: `$GIMLI_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.gimli/logs/...`; profiles use `~/.gimli-<profile>/logs/...`)<br />Linux: `journalctl --user -u gimli-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "Gimli Gateway (<profile>)" /V /FO LIST` |
| Session files | `$GIMLI_STATE_DIR/agents/<agentId>/sessions/` |
| Media cache | `$GIMLI_STATE_DIR/media/` |
| Credentials | `$GIMLI_STATE_DIR/credentials/` |
...
trash "${GIMLI_STATE_DIR:-$HOME/.gimli}"
gimli channels login         # re-pair WhatsApp
gimli gateway restart           # or: gimli gateway
```
...
1. Check logs first: `/tmp/gimli/` (default: `gimli-YYYY-MM-DD.log`, or your configured `logging.file`)
