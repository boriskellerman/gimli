...
- `/usage cost` in chats: local cost summary aggregated from Gimli session logs.
- CLI: `gimli status --usage` prints a full per-provider breakdown.
- CLI: `gimli channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
