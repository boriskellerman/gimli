---
summary: "Chrome extension: let Gimli drive your existing Chrome tab"
read_when:
...
The Gimli Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate gimli-managed Chrome profile.
...
Gimli then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Gimli release (npm package) as static files. There is no separate “build” step.

After upgrading Gimli:
- Re-run `gimli browser extension install` to refresh the installed files under your Gimli state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Gimli ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Gimli to control.
...
- `ON`: attached; Gimli can drive that tab.
...
Debugging: `gimli sandbox explain`
...
`gimli browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `gimli browser extension install` first to copy the extension to a stable location under your Gimli state directory.
...
- **This is not isolated** like the dedicated gimli-managed profile.
