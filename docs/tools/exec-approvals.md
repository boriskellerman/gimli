...
- **gateway host** → `gimli` process on the gateway machine
...
`~/.gimli/exec-approvals.json` directly.

CLI: `gimli approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
