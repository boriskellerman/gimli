---
summary: "Agent tool surface for Gimli (browser, canvas, nodes, message, cron) replacing legacy `gimli-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `gimli-*` skills
---

# Tools (Gimli)

Gimli exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `gimli-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `gimli.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Gimli logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`gimli node run`).
...
Notes:
- Requires a Brave API key (recommended: `gimli configure --section web`, or set `BRAVE_API_KEY`).
...
### `browser`
Control the dedicated gimli browser.
...
- Quick smoke: `gimli nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
Core actions:
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `gimli gateway` restart in-place)
