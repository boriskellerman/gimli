...
You can also set `GIMLI_VERBOSE=1` instead of the flag.
...
### `docs.gimli.bot` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.gimli.bot` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.gimli.bot` to the allowlist, then retry.
...
- Check the allowlist: `gimli config get agents.defaults.models`
...
If you can, include the relevant log tail from `gimli logs --follow`.
