...
Broadcast groups are evaluated after channel allowlists and group activation rules. In WhatsApp groups, this means broadcasts happen when Gimli would normally reply (for example: on mention, depending on your group settings).
...
**Result:** When Gimli would reply in this chat, it will run all three agents.
...
Workspace: /Users/pascal/gimli-alfred/
...
Workspace: /Users/pascal/gimli-baerbel/
Tools: read only
```
...
**Debug:**
```bash
tail -f ~/.gimli/logs/gateway.log | grep broadcast
```
