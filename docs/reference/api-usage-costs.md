...
Gimli features that can generate provider usage or paid API calls.
...
- `gimli status --usage` and `gimli channels list` show provider **usage windows**
...
Gimli can pick up credentials from:
...
### 8) Model scan / probe
`gimli models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
