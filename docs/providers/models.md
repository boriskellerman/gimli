---
summary: "Model providers (LLMs) supported by Gimli"
read_when:
...
Gimli can use many LLM providers. Pick one, authenticate, then set the default
...
1) Authenticate with the provider (usually via `gimli onboard`).
