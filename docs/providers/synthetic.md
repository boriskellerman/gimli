---
summary: "Use Synthetic's Anthropic-compatible API in Gimli"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Gimli registers it as the
...
```bash
gimli onboard --auth-choice synthetic-api-key
```
...
Note: Gimli's Anthropic client appends `/v1` to the base URL, so use
