---
summary: "Use Venice AI privacy-focused models in Gimli"
read_when:
  - You want privacy-focused inference in Gimli
...
```bash
gimli onboard --auth-choice venice-api-key
```
...
```bash
gimli chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Gimli shows all available Venice models. Pick based on your needs:
...
```bash
gimli models set venice/claude-opus-45
gimli models set venice/llama-3.3-70b
```

List all available models:

```bash
gimli models list | grep venice
```

## Configure via `gimli configure`

1. Run `gimli configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Gimli automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
gimli chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
gimli chat --model venice/claude-opus-45

# Use uncensored model
gimli chat --model venice/venice-uncensored

# Use vision model with image
gimli chat --model venice/qwen3-vl-235b-a22b

# Use coding model
gimli chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `gimli models list` to see currently available models. Some models may be temporarily offline.
