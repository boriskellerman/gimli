---
summary: "First-run onboarding flow for Gimli (macOS app)"
read_when:
...
- Writes credentials to `~/.gimli/credentials/oauth.json`
...
The app can install the global `gimli` CLI via npm/pnpm so terminal
...
On the first agent run, Gimli bootstraps a workspace (default `~/gimli`):
...
```bash
gimli webhooks gmail setup --account you@gmail.com
```
