...
read_when:
  - You are building a Gimli plugin
  - You need to ship a plugin config schema or debug plugin validation errors
---
# Plugin manifest (gimli.plugin.json)

Every plugin **must** ship a `gimli.plugin.json` file in the **plugin root**.
Gimli uses this manifest to validate configuration **without executing plugin
