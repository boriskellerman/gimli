---
summary: "Gimli on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Gimli on a Raspberry Pi
  - Running Gimli on ARM devices
  - Building a cheap always-on personal AI
---

# Gimli on Raspberry Pi

## Goal

Run a persistent, always-on Gimli Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# Update config
gimli config set gateway.bind tailnet
...
Most Gimli features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled gimli

# Enable if not
sudo systemctl enable gimli

# Start on boot
sudo systemctl start gimli
```
...
```bash
# Check logs
journalctl -u gimli --no-pager -n 100

# Common fix: rebuild
cd ~/gimli  # if using hackable install
npm run build
sudo systemctl restart gimli
```
