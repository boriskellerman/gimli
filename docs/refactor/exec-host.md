...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.gimli/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.gimli/node.json` (node id + pairing token).
...
Path: `~/.gimli/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.gimli/exec-approvals.sock",
...
### IPC
- Unix socket at `~/.gimli/exec-approvals.sock` (0600).
