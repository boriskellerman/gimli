...
Delivery:
- Publish as `@gimli/plugin-sdk` (or export from core under `gimli/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `GimliPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: GimliConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: GimliConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: GimliConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: GimliConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: GimliConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: GimliConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@gimli/plugin-sdk`.
- Add `api.runtime` to `GimliPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `gimliRuntime: ">=2026.2.0"`).
