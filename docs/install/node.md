...
read_when:
  - "You installed Gimli but `gimli` is “command not found”"
...
Gimli’s runtime baseline is **Node 22+**.

If you can run `npm install -g gimli@latest` but later see `gimli: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
