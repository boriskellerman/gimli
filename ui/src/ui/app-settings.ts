...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") await loadPresence(host as unknown as GimliApp);
  if (host.tab === "sessions") await loadSessions(host as unknown as GimliApp);
  if (host.tab === "cron") await loadCron(host);
  if (host.tab === "skills") await loadSkills(host as unknown as GimliApp);
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as GimliApp);
    await loadDevices(host as unknown as GimliApp);
    await loadConfig(host as unknown as GimliApp);
    await loadExecApprovals(host as unknown as GimliApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as GimliApp);
    await loadConfig(host as unknown as GimliApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as GimliApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as GimliApp, { reset: true });
    scheduleLogsScroll(
...
export function inferBasePath() {
  ...
  const configured = window.__GIMLI_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as GimliApp, false),
    loadPresence(host as unknown as GimliApp),
    loadSessions(host as unknown as GimliApp),
    loadCronStatus(host as unknown as GimliApp),
    loadDebug(host as unknown as GimliApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as GimliApp, true),
    loadConfigSchema(host as unknown as GimliApp),
    loadConfig(host as unknown as GimliApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as GimliApp, false),
    loadCronStatus(host as unknown as GimliApp),
    loadCronJobs(host as unknown as GimliApp),
  ]);
}
