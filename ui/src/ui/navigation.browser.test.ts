...
import { GimliApp } from "./app";
import "../styles.css";

const originalConnect = GimliApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("gimli-app") as GimliApp;
...
beforeEach(() => {
  GimliApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__GIMLI_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  GimliApp.prototype.connect = originalConnect;
  window.__GIMLI_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/gimli/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/gimli");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/gimli/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__GIMLI_CONTROL_UI_BASE_PATH__ = "/gimli";
    const app = mountApp("/gimli/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/gimli");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/gimli/sessions");
  });
