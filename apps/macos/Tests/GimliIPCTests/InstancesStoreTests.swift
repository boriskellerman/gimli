...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: GimliProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: GimliProtocol.AnyCodable] = [
            "presence": .init([GimliProtocol.AnyCodable(entry)]),
        ]
        let payload = GimliProtocol.AnyCodable(payloadMap)
