...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: GimliProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: GimliKit.AnyCodable]()) {
                $0[$1.key] = GimliKit.AnyCodable($1.value.base)
            }
    ...
    private func friendlyGatewayMessage(_ error: Error) -> String {
           ...
           urlErr.code == .dataNotAllowed // used for WS close 1008 auth failures
        {
            ...
            let tokenKey = CommandResolver.connectionModeIsRemote()
                ...
                : "gateway.auth.token"
            return
                "Gateway rejected token; set \(tokenKey) (or GIMLI_GATEWAY_TOKEN) " +
    ...
    private static func bridgeToProtocolArgs(
        _ value: GimliProtocol.AnyCodable?) -> [String: GimliProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: GimliProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: GimliKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: GimliProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("gimli.control.heartbeat")
    static let controlAgentEvent = Notification.Name("gimli.control.agent")
}
