...
enum CommandResolver {
    private static let projectRootDefaultsKey = "gimli.gatewayProjectRootPath"
    private static let helperName = "gimli"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let binEntry = root.appendingPathComponent("bin/gimli.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/gimli")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let gimliPaths = self.gimliManagedPaths(home: home)
        if !gimliPaths.isEmpty {
            extras.insert(contentsOf: gimliPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + gimliPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func gimliManagedPaths(home: URL) -> [String] {
        let base = home.appendingPathComponent(".gimli")
    ...
    static func gimliExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectGimliExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        let candidate = self.projectRoot().appendingPathComponent("bin/gimli.js").path
        return FileManager().isReadableFile(atPath: candidate) ? candidate : nil
    }

    static func hasAnyGimliInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.gimliExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            ...
            if let gimliPath = self.projectGimliExecutable(projectRoot: root) {
                return [gimliPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "gimli", subcommand] + extraArgs
            }
            if let gimliPath = self.gimliExecutable(searchPaths: searchPaths) {
                return [gimliPath, subcommand] + extraArgs
            }

            let missingEntry = """
            gimli entrypoint missing (looked for dist/index.js or bin/gimli.js); run pnpm build.
            """
    ...
    // Existing callers still refer to gimliCommand; keep it as node alias.
    static func gimliCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real gimli CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/gimli"
        ...
        \(cliSection)
        \(projectSection)
        if command -v gimli >/dev/null 2>&1; then
          CLI="$(command -v gimli)"
          gimli \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/gimli.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/gimli.js"
            node "$PRJ/bin/gimli.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent gimli"
          pnpm --silent gimli \(quotedArgs);
        else
          echo "gimli CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? GimliConfigFile.loadDict()
