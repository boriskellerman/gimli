...
enum SessionMenuPreviewLoader {
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> GimliSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: GimliSessionPreviewEntry,
    ...
    private static func cache(payload: GimliSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: GimliChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [GimliKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [GimliKit.AnyCodable]) -> [GimliChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(GimliChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: GimliChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: GimliChatMessage) -> Bool {
    ...
    private static func toolNames(for message: GimliChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: GimliChatMessage) -> String? {
