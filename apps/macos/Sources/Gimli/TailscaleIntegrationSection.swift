...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.gimli/gimli.json. Prefer GIMLI_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.gimli/gimli.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.gimli/gimli.json. Restart the gateway to apply."
        }
