...
enum GimliLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = GimliOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = GimliFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        GimliLogging.bootstrapIfNeeded()
        let label = GimliLogging.makeLabel(subsystem: subsystem, category: category)
...
struct GimliOSLogHandler: LogHandler {
...
struct GimliFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = GimliLogging.parseLabel(self.label)
