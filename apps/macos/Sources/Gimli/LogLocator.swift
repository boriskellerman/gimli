...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["GIMLI_LOG_DIR"], !override.isEmpty {
            return URL(fileURLWithPath: override)
        }

        return URL(fileURLWithPath: "/tmp/gimli")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("gimli-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("gimli-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/gimli/ (rolling or stdout), or nil if none exist.
    static func bestLogFile() -> URL? {
        ...
        return files
            .filter { $0.lastPathComponent.hasPrefix("gimli") && $0.pathExtension == "log" }
            .max { lhs, rhs in
