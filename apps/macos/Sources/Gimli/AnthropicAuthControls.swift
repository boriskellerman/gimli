...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: GimliOAuthStore.AnthropicOAuthStatus = GimliOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: GimliOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = GimliOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = GimliOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try GimliOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Gimli can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: GimliOAuthStore.AnthropicOAuthStatus,
