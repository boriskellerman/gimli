...
struct MacGatewayChatTransport: GimliChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> GimliChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(GimliChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> GimliChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                GimliGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<GimliChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any GimliChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = GimliChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: GimliChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<GimliChatView>) -> NSViewController
    {
