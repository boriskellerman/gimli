...
struct GimliMacCLI {
    static func main() async {
        ...
        default:
            fputs("gimli-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      ...
      gimli-mac discover [--timeout <ms>] [--json] [--include-local]
      gimli-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      gimli-mac connect
      gimli-mac connect --url ws://127.0.0.1:18789 --json
      gimli-mac discover --timeout 3000 --json
      gimli-mac wizard --mode local
    """)
}
