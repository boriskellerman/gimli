...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [GimliCapability.canvas.rawValue, GimliCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(GimliCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(GimliCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = GimliLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(GimliCapability.location.rawValue) }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            GimliSystemCommand.notify.rawValue,
            GimliSystemCommand.which.rawValue,
            GimliSystemCommand.run.rawValue,
            GimliSystemCommand.execApprovalsGet.rawValue,
            GimliSystemCommand.execApprovalsSet.rawValue,
        ]

        let caps = Set(self.currentCaps())
        if caps.contains(GimliCapability.camera.rawValue) {
            commands.append(GimliCameraCommand.list.rawValue)
            commands.append(GimliCameraCommand.snap.rawValue)
            commands.append(GimliCameraCommand.clip.rawValue)
        }
        if caps.contains(GimliCapability.location.rawValue) {
            commands.append(GimliLocationCommand.get.rawValue)
        }
