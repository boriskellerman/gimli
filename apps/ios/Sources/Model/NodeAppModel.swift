...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = GimliCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = GimliCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = GimliCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = GimliCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = GimliCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__gimli__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: GimliLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for gimli:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case GimliLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case GimliCanvasCommand.present.rawValue,
                 GimliCanvasCommand.hide.rawValue,
                 GimliCanvasCommand.navigate.rawValue,
                 GimliCanvasCommand.evalJS.rawValue,
                 GimliCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case GimliCanvasA2UICommand.reset.rawValue,
                 GimliCanvasA2UICommand.push.rawValue,
                 GimliCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case GimliCameraCommand.list.rawValue,
                 GimliCameraCommand.snap.rawValue,
                 GimliCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case GimliScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: GimliNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: GimliNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(GimliLocationGetParams.self, from: req.paramsJSON)) ??
            GimliLocationGetParams()
        ...
        let payload = GimliLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case GimliCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(GimliCanvasPresentParams.self, from: req.paramsJSON)) ??
                GimliCanvasPresentParams()
        ...
        case GimliCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case GimliCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(GimliCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case GimliCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(GimliCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case GimliCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(GimliCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case GimliCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              if (!globalThis.gimliA2UI) return JSON.stringify({ ok: false, error: "missing gimliA2UI" });
              return JSON.stringify(globalThis.gimliA2UI.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case GimliCanvasA2UICommand.push.rawValue, GimliCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == GimliCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(GimliCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try GimliCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(GimliCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try GimliCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                if (!globalThis.gimliA2UI) return JSON.stringify({ ok: false, error: "missing gimliA2UI" });
                const messages = \(messagesJSON);
                return JSON.stringify(globalThis.gimliA2UI.applyMessages(messages));
              } catch (e) {
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case GimliCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case GimliCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(GimliCameraSnapParams.self, from: req.paramsJSON)) ??
                GimliCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case GimliCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(GimliCameraClipParams.self, from: req.paramsJSON)) ??
                GimliCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(GimliScreenRecordParams.self, from: req.paramsJSON)) ??
            GimliScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> GimliLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return GimliLocationMode(rawValue: raw) ?? .off
    }
