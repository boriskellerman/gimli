...
struct IOSGatewayChatTransport: GimliChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> GimliChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(GimliChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> GimliChatHistoryPayload {
        ...
        return try JSONDecoder().decode(GimliChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [GimliChatAttachmentPayload]) async throws -> GimliChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [GimliChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(GimliChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(GimliGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: GimliGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: GimliChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: GimliAgentEventPayload.self)
                        {
