...
private final class TestChatTransport: @unchecked Sendable, GimliChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [GimliChatHistoryPayload]
    private let sessionsResponses: [GimliChatSessionsListResponse]

    private let stream: AsyncStream<GimliChatTransportEvent>
    private let continuation: AsyncStream<GimliChatTransportEvent>.Continuation

    init(
        historyResponses: [GimliChatHistoryPayload],
        sessionsResponses: [GimliChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<GimliChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> GimliChatHistoryPayload {
        ...
        return self.historyResponses.last ?? GimliChatHistoryPayload(
        ...
        attachments _: [GimliChatAttachmentPayload]) async throws -> GimliChatSendResponse
    {
        ...
        return GimliChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> GimliChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? GimliChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: GimliChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = GimliChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = GimliChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { GimliChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = GimliChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = GimliChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = GimliChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { GimliChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
