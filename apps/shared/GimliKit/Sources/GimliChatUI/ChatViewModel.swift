...
private let chatUILogger = Logger(subsystem: "bot.molt", category: "GimliChatUI")

@MainActor
@Observable
public final class GimliChatViewModel {
    public private(set) var messages: [GimliChatMessage] = []
    ...
    public var attachments: [GimliPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [GimliChatPendingToolCall] = []
    public private(set) var sessions: [GimliChatSessionEntry] = []
    private let transport: any GimliChatTransport
    ...
    private var pendingToolCallsById: [String: GimliChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any GimliChatTransport) {
    ...
    public var sessionChoices: [GimliChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [GimliChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [GimliChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: GimliPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [GimliChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: GimliChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [GimliChatMessage]) -> [GimliChatMessage] {
        var result: [GimliChatMessage] = []
    ...
    private static func dedupeKey(for message: GimliChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [GimliChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> GimliChatAttachmentPayload in
            GimliChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> GimliChatSessionEntry {
        GimliChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: GimliChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: GimliChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: GimliAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = GimliChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> GimliPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
