...
@MainActor
struct GimliChatComposer: View {
    @Bindable var viewModel: GimliChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(GimliChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(GimliChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \GimliPendingAttachment.id)
                { (att: GimliPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            GimliPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(GimliChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(GimliChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(GimliChatTheme.subtleCard)
        .clipShape(Capsule())
    }
