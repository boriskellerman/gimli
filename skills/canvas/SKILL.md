...
Display HTML content on connected Gimli nodes (Mac app, iOS, Android).
...
```
http://<tailscale-hostname>:18793/__gimli__/canvas/<file>.html
```
...
Place files in the canvas root directory (default `~/gimli/canvas/`):

```bash
cat > ~/gimli/canvas/my-game.html << 'HTML'
<!DOCTYPE html>
<html>
...
```bash
cat ~/.gimli/gimli.json | jq '.gateway.bind'
```

Then construct the URL:
- **loopback**: `http://127.0.0.1:18793/__gimli__/canvas/<file>.html`
- **lan/tailnet/auto**: `http://<hostname>:18793/__gimli__/canvas/<file>.html`
...
**Example:**
```
canvas action:present node:mac-63599bc4-b54d-4392-9048-b97abd58343a target:http://peters-mac-studio-1.sheep-coho.ts.net:18793/__gimli__/canvas/snake.html
```
...
**Debug steps:**
1. Check server bind: `cat ~/.gimli/gimli.json | jq '.gateway.bind'`
2. Check what port canvas is on: `lsof -i :18793`
3. Test URL directly: `curl http://<hostname>:18793/__gimli__/canvas/<file>.html`
...
Node is offline. Use `gimli nodes list` to find online nodes.
...
The canvas host serves from `/__gimli__/canvas/` prefix:

```
http://<host>:18793/__gimli__/canvas/index.html  → ~/gimli/canvas/index.html
http://<host>:18793/__gimli__/canvas/games/snake.html → ~/gimli/canvas/games/snake.html
```

The `/__gimli__/canvas/` prefix is defined by `CANVAS_HOST_PATH` constant.
