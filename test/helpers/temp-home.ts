...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("GIMLI_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.GIMLI_STATE_DIR = path.join(base, ".gimli");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "gimli-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".gimli", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
