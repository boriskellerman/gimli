...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `gimli-e2e-${name}-`));
  const configDir = path.join(homeDir, ".gimli");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          HOME: homeDir,
          GIMLI_CONFIG_PATH: configPath,
          GIMLI_STATE_DIR: stateDir,
          GIMLI_GATEWAY_TOKEN: "",
          GIMLI_GATEWAY_PASSWORD: "",
          GIMLI_SKIP_CHANNELS: "1",
          GIMLI_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        GIMLI_GATEWAY_TOKEN: inst.gatewayToken,
        GIMLI_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          GIMLI_GATEWAY_PORT: String(gwA.port),
          GIMLI_GATEWAY_TOKEN: gwA.gatewayToken,
          GIMLI_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          GIMLI_GATEWAY_PORT: String(gwB.port),
          GIMLI_GATEWAY_TOKEN: gwB.gatewayToken,
          GIMLI_GATEWAY_PASSWORD: "",
        }),
