...
import type { GimliConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("gimli-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("gimli-bin-sherpa-");
      const modelDir = await makeTempDir("gimli-sherpa-model-");
      ...
      const cfg: GimliConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("gimli-bin-whispercpp-");
      const modelDir = await makeTempDir("gimli-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("gimli-bin-gemini-");
      ...
      const cfg: GimliConfig = { tools: { media: { image: {} } } };
