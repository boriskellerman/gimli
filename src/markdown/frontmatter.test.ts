...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { gimli?: { emoji?: string } };
    expect(parsed.gimli?.emoji).toBe("disk");
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { gimli?: { events?: string[] } };
