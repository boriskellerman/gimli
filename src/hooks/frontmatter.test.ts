...
`;
    ...
    expect(parsed.gimli.emoji).toBe("💾");
    expect(parsed.gimli.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata as string);
    expect(parsed.gimli.emoji).toBe("📝");
    expect(parsed.gimli.events).toEqual(["command"]);
    expect(parsed.gimli.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.gimli.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"gimli": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"gimli": {"events": ["test"]}}');
  });
...
describe("resolveGimliMetadata", () => {
  it("extracts gimli metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveGimliMetadata(frontmatter);
  ...
  it("returns undefined when gimli key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        gimli: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Gimli" },
            { id: "npm", kind: "npm", package: "@gimli/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@gimli/hook");
  });
    ...
    "gimli":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Gimli" }],
      },
  }
---
...
`;
    ...
    const gimli = resolveGimliMetadata(frontmatter);
    expect(gimli).toBeDefined();
    expect(gimli?.emoji).toBe("💾");
    expect(gimli?.events).toEqual(["command:new"]);
    expect(gimli?.requires?.config).toEqual(["workspace.dir"]);
    expect(gimli?.install?.[0].kind).toBe("bundled");
  });
