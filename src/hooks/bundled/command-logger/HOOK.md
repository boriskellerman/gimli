...
homepage: https://docs.gimli.bot/hooks#command-logger
metadata:
  {
    "gimli":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.gimli/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.gimli/logs/commands.log ~/.gimli/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/gimli`:
   ```
   /home/username/.gimli/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
