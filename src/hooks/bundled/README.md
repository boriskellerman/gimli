...
This directory contains hooks that ship with Gimli. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.gimli.bot/hooks/soul-evil
...
metadata:
  { "gimli": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
Hooks can be configured in `~/.gimli/gimli.json`:
...
2. Restart gateway: `pkill -9 -f 'gimli.*gateway' && pnpm gimli gateway`
3. Enable the hook: `gimli hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.gimli.bot/hooks
