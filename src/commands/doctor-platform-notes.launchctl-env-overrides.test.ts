...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "GIMLI_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("GIMLI_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv GIMLI_GATEWAY_TOKEN");
    expect(message).not.toContain("GIMLI_GATEWAY_PASSWORD");
  });
