...
export function listAgentEntries(cfg: GimliConfig): AgentEntry[] {
...
function resolveAgentName(cfg: GimliConfig, agentId: string) {
...
function resolveAgentModel(cfg: GimliConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: GimliConfig): AgentSummary[] {
