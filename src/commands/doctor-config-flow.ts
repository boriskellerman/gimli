...
function stripUnknownConfigKeys(config: GimliConfig): {
  config: GimliConfig;
  removed: string[];
} {
  const parsed = GimliSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }

  const next = structuredClone(config) as GimliConfig;
...
function noteOpencodeProviderOverrides(cfg: GimliConfig) {
...
function hasExplicitConfigPath(env: NodeJS.ProcessEnv): boolean {
  return Boolean(env.GIMLI_CONFIG_PATH?.trim() || env.GIMLI_CONFIG_PATH?.trim());
}
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (!hasExplicitConfigPath(process.env) && snapshot.exists) {
    ...
    if (basename === "gimli.json") {
      const canonicalPath = path.join(path.dirname(snapshot.path), "gimli.json");
  ...
  let cfg: GimliConfig = baseCfg;
  let candidate = structuredClone(baseCfg) as GimliConfig;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("gimli doctor --fix")}" to apply legacy migrations.`);
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("gimli doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "gimli doctor --fix" to remove these keys.');
    }
  }
