...
const requireAgentDir = () => {
  const agentDir = process.env.GIMLI_AGENT_DIR;
  if (!agentDir) throw new Error("GIMLI_AGENT_DIR not set");
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.GIMLI_STATE_DIR;
  const previousAgentDir = process.env.GIMLI_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.GIMLI_AGENT_DIR;
    } else {
      process.env.GIMLI_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.GIMLI_OAUTH_DIR;
  });

  it("writes auth-profiles.json under GIMLI_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "gimli-oauth-"));
    process.env.GIMLI_STATE_DIR = tempStateDir;
    process.env.GIMLI_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.GIMLI_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to GIMLI_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "gimli-minimax-"));
    process.env.GIMLI_STATE_DIR = tempStateDir;
    process.env.GIMLI_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.GIMLI_AGENT_DIR;
