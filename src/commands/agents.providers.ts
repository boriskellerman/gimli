...
function resolveDefaultAccountId(cfg: GimliConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: GimliConfig): boolean {
...
export function summarizeBindings(cfg: GimliConfig, bindings: AgentBinding[]): string[] {
