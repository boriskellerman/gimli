...
beforeEach(() => {
  ...
  resolveGimliPackageRoot.mockReset().mockResolvedValue(null);
  ...
  originalUpdateInProgress = process.env.GIMLI_UPDATE_IN_PROGRESS;
  process.env.GIMLI_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "gimli-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalUpdateInProgress === undefined) {
    ...
    process.env.GIMLI_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveGimliPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadGimliPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/gimli-root.js", () => ({
  resolveGimliPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "gimli-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.GIMLI_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when GIMLI_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/gimli.json",
      exists: true,
      raw: "{}",
    ...
    process.env.GIMLI_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
