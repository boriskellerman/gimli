...
const SEARCH_TOOL = "https://docs.gimli.bot/mcp.SearchGimli";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.gimli.bot");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('gimli docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.gimli.bot/");
      runtime.log(`Search: ${formatCliCommand('gimli docs "your query"')}`);
    }
    return;
  }
