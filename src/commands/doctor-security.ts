...
import type { GimliConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: GimliConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("gimli security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("gimli configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("gimli config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("gimli doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("gimli config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
