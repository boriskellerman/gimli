...
function resolveConfiguredModelRaw(cfg: GimliConfig): string {
  const raw = cfg.agents?.defaults?.model as { primary?: string } | string | undefined;
  if (typeof raw === "string") return raw.trim();
  return raw?.primary?.trim() ?? "";
}

function resolveConfiguredModelKeys(cfg: GimliConfig): string[] {
...
export function applyPrimaryModel(cfg: GimliConfig, model: string): GimliConfig {
...
export function applyModelAllowlist(cfg: GimliConfig, models: string[]): GimliConfig {
  const defaults = cfg.agents?.defaults;
