...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Gimli status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("gimli security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("gimli security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.gimli.bot/faq");
  runtime.log("Troubleshooting: https://docs.gimli.bot/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("gimli status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("gimli logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("gimli status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("gimli gateway probe")}`);
  }
}
