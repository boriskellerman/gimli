...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveGimliAgentDir: vi.fn().mockReturnValue("/tmp/gimli-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/gimli-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveGimliAgentDir: mocks.resolveGimliAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/gimli-agent/auth-profiles.json");
