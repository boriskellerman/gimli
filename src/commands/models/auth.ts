...
import { readConfigFileSnapshot, type GimliConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: GimliConfig, model: string): GimliConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("gimli plugins install")}\`.`,
    );
  }
