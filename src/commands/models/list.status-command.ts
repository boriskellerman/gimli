...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("gimli models auth setup-token")}\` or \`${formatCliCommand("gimli configure")}\`.`
          : `Run \`${formatCliCommand("gimli configure")}\` or set an API key env var.`;
