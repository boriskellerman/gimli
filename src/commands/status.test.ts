...
beforeAll(() => {
  previousProfile = process.env.GIMLI_PROFILE;
  process.env.GIMLI_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.GIMLI_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/gimli-root.js", () => ({
  resolveGimliPackageRoot: vi.fn().mockResolvedValue("/tmp/gimli"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/gimli/pnpm-lock.yaml",
      markerPath: "/tmp/gimli/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Gimli status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("gimli status --all") ||
          l.includes("gimli --profile isolated status --all") ||
          l.includes("gimli status --all") ||
          l.includes("gimli --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.GIMLI_GATEWAY_TOKEN = "abcd1234";
    try {
