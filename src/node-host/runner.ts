...
import { ensureGimliCliOnPath } from "../infra/path-env.js";
...
const execHostEnforced = process.env.GIMLI_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed = process.env.GIMLI_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureGimliCliOnPath({ pathEnv: process.env.PATH ?? "" });
