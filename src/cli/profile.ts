...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.gimli${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.GIMLI_PROFILE = profile;

  const stateDir = env.GIMLI_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.GIMLI_STATE_DIR?.trim()) env.GIMLI_STATE_DIR = stateDir;

  if (!env.GIMLI_CONFIG_PATH?.trim()) {
    env.GIMLI_CONFIG_PATH = path.join(stateDir, "gimli.json");
  }

  if (profile === "dev" && !env.GIMLI_GATEWAY_PORT?.trim()) {
    env.GIMLI_GATEWAY_PORT = "19001";
  }
}
