...
function appendGimliHubHint(output: string, json?: boolean): string {
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("gimli skills list")}\` to see all skills.`
      : "No skills found.";
    return appendGimliHubHint(message, opts.json);
  }
  ...
  return appendGimliHubHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendGimliHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("gimli skills list")}\` to see available skills.`,
      opts.json,
    );
  }
...
export function registerSkillsCli(program: Command) {
  const skills = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/skills", "docs.gimli.bot/cli/skills")}\n`,
    );
