...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("gimli node install"), formatCliCommand("gimli node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("gimli node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    GIMLI_LOG_PREFIX: baseEnv.GIMLI_LOG_PREFIX ?? "node",
