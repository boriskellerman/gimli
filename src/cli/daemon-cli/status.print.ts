...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean }) {
  ...
  if (service.configAudit?.issues.length) {
    ...
    defaultRuntime.error(
      warnText(
        `Recommendation: run "${formatCliCommand("gimli doctor")}" (or "${formatCliCommand("gimli doctor --repair")}").`,
      ),
    );
  }
  ...
  if (status.config) {
    ...
    if (status.config.mismatch) {
      ...
      defaultRuntime.error(
        errorText(
          `Fix: rerun \`${formatCliCommand("gimli gateway install --force")}\` from the same --profile / GIMLI_STATE_DIR you expect.`,
        ),
      );
    }
    spacer();
  ...
  if (service.runtime?.cachedLabel) {
    ...
    const labelValue = resolveGatewayLaunchAgentLabel(env.GIMLI_PROFILE);
    defaultRuntime.error(
      errorText(
        `LaunchAgent label cached but plist missing. Clear with: launchctl bootout gui/$UID/${labelValue}`,
      ),
    );
    defaultRuntime.error(errorText(`Then reinstall: ${formatCliCommand("gimli gateway install")}`));
    spacer();
  }
  ...
  if (
    ...
    if (process.platform === "linux") {
      const env = (service.command?.environment ?? process.env) as NodeJS.ProcessEnv;
      const unit = resolveGatewaySystemdServiceName(env.GIMLI_PROFILE);
      defaultRuntime.error(
  ...
  if (legacyServices.length > 0) {
    ...
    defaultRuntime.error(errorText(`Cleanup: ${formatCliCommand("gimli doctor")}`));
    spacer();
  }
  ...
  defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("gimli status")}`);
  defaultRuntime.log(`${label("Troubleshooting:")} https://docs.gimli.bot/troubleshooting`);
}
