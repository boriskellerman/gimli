...
const STEP_LABELS: Record<string, string> = {
  ...
  "gimli doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "gimli";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "gimli"]);
const CLI_NAME = resolveCliName();
const GIMLI_REPO_URL = "https://github.com/gimli/gimli.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), "gimli");

function normalizeTag(value?: string | null): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  if (trimmed.startsWith("gimli@")) return trimmed.slice("gimli@".length);
...
function resolveGitInstallDir(): string {
  const override = process.env.GIMLI_GIT_DIR?.trim();
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", GIMLI_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `GIMLI_GIT_DIR points at a non-git directory: ${params.dir}. Set GIMLI_GIT_DIR to an empty folder or a gimli checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`GIMLI_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("Gimli update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Gimli..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Gimli install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("gimli doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("gimli gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g gimli@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g gimli@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.GIMLI_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.GIMLI_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("gimli gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("gimli doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("gimli gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("gimli gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `gimli update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `GIMLI_GIT_DIR points at a non-git directory: ${gitDir}. Set GIMLI_GIT_DIR to an empty folder or a gimli checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via GIMLI_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || ok === false) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Gimli to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["gimli update", "Update a source checkout (git)"],
        ["gimli update --channel beta", "Switch to beta channel (git + npm)"],
        ["gimli update --channel dev", "Switch to dev channel (git + npm)"],
        ["gimli update --tag beta", "One-off update to a dist-tag or version"],
        ["gimli update --no-restart", "Update without restarting the service"],
        ["gimli update --json", "Output result as JSON"],
        ["gimli update --yes", "Non-interactive (accept downgrade prompts)"],
        ["gimli update wizard", "Interactive update wizard"],
        ["gimli --update", "Shorthand for gimli update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run gimli update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.gimli.bot/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["gimli update status", "Show channel + version status."],
          ["gimli update status --json", "JSON output."],
          ["gimli update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.gimli.bot/cli/update")}`,
    )
    .action(async (opts) => {
      try {
