...
const SANDBOX_EXAMPLES = {
  main: [
    ["gimli sandbox list", "List all sandbox containers."],
    ["gimli sandbox list --browser", "List only browser containers."],
    ["gimli sandbox recreate --all", "Recreate all containers."],
    ["gimli sandbox recreate --session main", "Recreate a specific session."],
    ["gimli sandbox recreate --agent mybot", "Recreate agent containers."],
    ["gimli sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["gimli sandbox list", "List all sandbox containers."],
    ["gimli sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["gimli sandbox recreate --all", "Recreate all containers."],
    ["gimli sandbox recreate --session main", "Recreate a specific session."],
    ["gimli sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["gimli sandbox recreate --browser --all", "Recreate only browser containers."],
    ["gimli sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["gimli sandbox explain", "Show effective sandbox config."],
    ["gimli sandbox explain --session agent:main:main", "Explain a specific session."],
    ["gimli sandbox explain --agent work", "Explain an agent sandbox."],
    ["gimli sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.gimli.bot/cli/sandbox")}\n`,
    )
    .action(() => {
