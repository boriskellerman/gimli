...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    GIMLI_STATE_DIR: process.env.GIMLI_STATE_DIR,
    GIMLI_CONFIG_PATH: process.env.GIMLI_CONFIG_PATH,
    GIMLI_GATEWAY_PORT: process.env.GIMLI_GATEWAY_PORT,
    GIMLI_PROFILE: process.env.GIMLI_PROFILE,
  };

  beforeEach(() => {
    process.env.GIMLI_STATE_DIR = "/tmp/gimli-cli-state";
    process.env.GIMLI_CONFIG_PATH = "/tmp/gimli-cli-state/gimli.json";
    delete process.env.GIMLI_GATEWAY_PORT;
    delete process.env.GIMLI_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.GIMLI_STATE_DIR !== undefined)
      process.env.GIMLI_STATE_DIR = originalEnv.GIMLI_STATE_DIR;
    else delete process.env.GIMLI_STATE_DIR;

    if (originalEnv.GIMLI_CONFIG_PATH !== undefined)
      process.env.GIMLI_CONFIG_PATH = originalEnv.GIMLI_CONFIG_PATH;
    else delete process.env.GIMLI_CONFIG_PATH;

    if (originalEnv.GIMLI_GATEWAY_PORT !== undefined)
      process.env.GIMLI_GATEWAY_PORT = originalEnv.GIMLI_GATEWAY_PORT;
    else delete process.env.GIMLI_GATEWAY_PORT;

    if (originalEnv.GIMLI_PROFILE !== undefined)
      process.env.GIMLI_PROFILE = originalEnv.GIMLI_PROFILE;
    else delete process.env.GIMLI_PROFILE;
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        GIMLI_PROFILE: "dev",
        GIMLI_STATE_DIR: "/tmp/gimli-daemon-state",
        GIMLI_CONFIG_PATH: "/tmp/gimli-daemon-state/gimli.json",
        GIMLI_GATEWAY_PORT: "19001",
      },
