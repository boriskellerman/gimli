...
${formatHelpExamples([
  ['gimli agent --to +15555550123 --message "status update"', "Start a new session."],
  ['gimli agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
  ...
  ['gimli agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.gimli.bot/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.gimli.bot/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "gimli agents set-identity --identity-file ~/gimli/IDENTITY.md --agent main",
