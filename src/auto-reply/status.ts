...
type AgentConfig = Partial<NonNullable<NonNullable<GimliConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.gimli/sessions/<SessionId>.jsonl)
...
const formatVoiceModeLine = (config?: GimliConfig, sessionEntry?: SessionEntry): string | null => {
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 Gimli ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: GimliConfig): string {
  const lines = ["ℹ️ Help", ""];
