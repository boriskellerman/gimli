...
import { scheduleGatewaySigusr1Restart, triggerGimliRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting gimli in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerGimliRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting gimli via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
