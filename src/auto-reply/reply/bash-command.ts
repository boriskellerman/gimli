...
function resolveForegroundMs(cfg: GimliConfig): number {
...
function formatElevatedUnavailableMessage(params: {
  ...
  if (params.sessionKey) {
    lines.push(`See: ${formatCliCommand(`gimli sandbox explain --session ${params.sessionKey}`)}`);
  }
  return lines.join("\n");
}
...
export async function handleBashChatCommand(params: {
  ...
  if (params.cfg.commands?.bash !== true) {
    return {
      text: "⚠️ bash is disabled. Set commands.bash=true to enable. Docs: https://docs.gimli.bot/tools/slash-commands#config",
    };
  }
