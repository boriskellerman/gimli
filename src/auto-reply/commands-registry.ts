...
export function isCommandEnabled(cfg: GimliConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: GimliConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: GimliConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: GimliConfig) {
