...
export function listSignalAccountIds(cfg: GimliConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: GimliConfig): string {
...
function mergeSignalAccountConfig(cfg: GimliConfig, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: GimliConfig): ResolvedSignalAccount[] {
