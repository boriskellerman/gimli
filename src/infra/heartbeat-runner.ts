...
function resolveActiveHoursTimezone(cfg: GimliConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: GimliConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: GimliConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: GimliConfig, agentId?: string): boolean {
...
function resolveHeartbeatConfig(cfg: GimliConfig, agentId?: string): HeartbeatConfig | undefined {
...
function resolveHeartbeatAgents(cfg: GimliConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: GimliConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: GimliConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
function resolveHeartbeatSession(cfg: GimliConfig, agentId?: string, heartbeat?: HeartbeatConfig) {
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: GimliConfig) => {
    if (state.stopped) return;
    const now = Date.now();
