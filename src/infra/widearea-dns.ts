...
export const WIDE_AREA_DISCOVERY_DOMAIN = "gimli.internal.";
export const WIDE_AREA_ZONE_FILENAME = "gimli.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*gimli-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "gimli";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "gimli");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "gimli-gw");
  ...
  records.push(`_gimli-gw._tcp IN PTR ${instanceLabel}._gimli-gw._tcp`);
  records.push(`${instanceLabel}._gimli-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._gimli-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; gimli-content-hash: ${contentHash}\n${contentBody}`;
}
