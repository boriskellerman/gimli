...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "gimli-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "gimli doctor",
        managerScriptArgs(manager, "gimli", ["doctor", "--non-interactive"]),
        gitRoot,
        { GIMLI_UPDATE_IN_PROGRESS: "1" },
      ),
    );
