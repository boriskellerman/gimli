...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: GimliConfig | null = null;

  constructor(private readonly ttlMs: number) {}

  get(key: string, cfg: GimliConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: GimliConfig): void {
    this.resetIfConfigChanged(cfg);
  ...
  clear(cfg?: GimliConfig): void {
    this.cache.clear();
    if (cfg) this.lastConfigRef = cfg;
  }

  private resetIfConfigChanged(cfg: GimliConfig): void {
