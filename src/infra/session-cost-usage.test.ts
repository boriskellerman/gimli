...
describe("session cost usage", () => {
  it("aggregates daily totals with log cost and pricing fallback", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "gimli-cost-"));
    ...
    const originalState = process.env.GIMLI_STATE_DIR;
    process.env.GIMLI_STATE_DIR = root;
    try {
      ...
      if (originalState === undefined) delete process.env.GIMLI_STATE_DIR;
      else process.env.GIMLI_STATE_DIR = originalState;
    }
  });

  it("summarizes a single session file", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "gimli-cost-session-"));
