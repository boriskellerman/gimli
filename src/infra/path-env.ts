...
function candidateBinDirs(opts: EnsureGimliPathOpts): string[] {
  ...
  // Bundled macOS app: `gimli` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingGimli = path.join(execDir, "gimli");
    if (isExecutable(siblingGimli)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/gimli` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "gimli"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `gimli` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureGimliCliOnPath(opts: EnsureGimliPathOpts = {}) {
  if (isTruthyEnvValue(process.env.GIMLI_PATH_BOOTSTRAPPED)) return;
  process.env.GIMLI_PATH_BOOTSTRAPPED = "1";
