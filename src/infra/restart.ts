...
export function triggerGimliRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(process.env.GIMLI_SYSTEMD_UNIT, process.env.GIMLI_PROFILE);
  ...
  const label =
    process.env.GIMLI_LAUNCHD_LABEL || resolveGatewayLaunchAgentLabel(process.env.GIMLI_PROFILE);
