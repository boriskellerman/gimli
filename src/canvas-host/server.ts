...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      (window.webkit.messageHandlers.gimliCanvasA2UIAction ||
        window.webkit.messageHandlers.gimliCanvasA2UIAction)
    );
  const hasAndroid = () =>
    !!(
      (window.gimliCanvasA2UIAction &&
        typeof window.gimliCanvasA2UIAction.postMessage === "function") ||
      (window.gimliCanvasA2UIAction &&
        typeof window.gimliCanvasA2UIAction.postMessage === "function")
    );
  const legacySend = typeof window.gimliSendUserAction === "function" ? window.gimliSendUserAction : undefined;
  if (!window.gimliSendUserAction && legacySend) {
    window.gimliSendUserAction = legacySend;
  }
  if (!window.gimliSendUserAction && typeof window.gimliSendUserAction === "function") {
    window.gimliSendUserAction = window.gimliSendUserAction;
  }
  const hasHelper = () =>
    typeof window.gimliSendUserAction === "function" ||
    typeof window.gimliSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("gimli:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Gimli node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.gimliSendUserAction === "function"
        ? window.gimliSendUserAction
        : window.gimliSendUserAction;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.GIMLI_SKIP_CANVAS_HOST)) return true;
...
export async function createCanvasHostHandler(
  ...
  const rootDir = resolveUserPath(opts.rootDir ?? path.join(os.homedir(), "gimli", "canvas"));
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Gimli Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
