...
import type { GimliConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: GimliConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: GimliConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = GimliSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as GimliConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as GimliConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as GimliConfig)),
    ),
  };
}
