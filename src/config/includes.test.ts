...
const ETC_GIMLI_DIR = path.join(ROOT_DIR, "etc", "gimli");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "gimli.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcGimliPath(...parts: string[]) {
  return path.join(ETC_GIMLI_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcGimliPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "gimli.json"))).toEqual({
      shared: true,
    });
  });
});
