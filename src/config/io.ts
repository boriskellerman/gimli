...
import type { GimliConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareGimliVersions } from "./version.js";
...
function coerceConfig(value: unknown): GimliConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as GimliConfig;
}
...
function stampConfigVersion(cfg: GimliConfig): GimliConfig {
...
function warnIfConfigFromFuture(cfg: GimliConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) return;
  const cmp = compareGimliVersions(VERSION, touched);
  if (cmp === null) return;
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Gimli (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: GimliConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): GimliConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as GimliConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as GimliConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: GimliConfig) {
    clearConfigCache();
...
// module scope. `GIMLI_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.GIMLI_CONFIG_CACHE_MS?.trim();
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.GIMLI_DISABLE_CONFIG_CACHE?.trim()) return false;
...
export function loadConfig(): GimliConfig {
...
export async function writeConfigFile(cfg: GimliConfig): Promise<void> {
  clearConfigCache();
