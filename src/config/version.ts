...
export function parseGimliVersion(raw: string | null | undefined): GimliVersion | null {
  if (!raw) return null;
...
export function compareGimliVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseGimliVersion(a);
  const parsedB = parseGimliVersion(b);
