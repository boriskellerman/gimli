...
function resolveChannelConfig(cfg: GimliConfig, channelId: string): Record<string, unknown> | null {
  const channels = cfg.channels as Record<string, unknown> | undefined;
  const entry = channels?.[channelId];
  return isRecord(entry) ? entry : null;
}

function isTelegramConfigured(cfg: GimliConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: GimliConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: GimliConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: GimliConfig): boolean {
...
function isIMessageConfigured(cfg: GimliConfig): boolean {
...
function isWhatsAppConfigured(cfg: GimliConfig): boolean {
...
function isGenericChannelConfigured(cfg: GimliConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: GimliConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: GimliConfig, providerId: string): boolean {
...
function resolveConfiguredPlugins(cfg: GimliConfig, env: NodeJS.ProcessEnv): PluginEnableChange[] {
...
function isPluginExplicitlyDisabled(cfg: GimliConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: GimliConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: GimliConfig, pluginId: string): GimliConfig {
...
function enablePluginEntry(cfg: GimliConfig, pluginId: string): GimliConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
