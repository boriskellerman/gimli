...
function setSlackDmPolicy(cfg: GimliConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Gimli";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Gimli`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: GimliConfig, allowFrom: string[]): GimliConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
