...
function setSignalDmPolicy(cfg: GimliConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function setSignalAllowFrom(cfg: GimliConfig, accountId: string, allowFrom: string[]): GimliConfig {
...
export const signalOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    await prompter.note(
      [
        'Link device with: signal-cli link -n "Gimli"',
        "Scan QR in Signal → Linked Devices",
        `Then run: ${formatCliCommand("gimli gateway call channels.status --params '{\"probe\":true}'")}`,
        `Docs: ${formatDocsLink("/signal", "signal")}`,
      ].join("\n"),
      "Signal next steps",
    );
