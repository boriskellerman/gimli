...
import { discoverGimliPlugins } from "../../plugins/discovery.js";
import type { PluginOrigin } from "../../plugins/types.js";
import type { GimliPackageManifest } from "../../plugins/manifest.js";
...
const ENV_CATALOG_PATHS = ["GIMLI_PLUGIN_CATALOG_PATHS", "GIMLI_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<GimliPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageGimli?: GimliPackageManifest;
}): ChannelPluginCatalogEntry | null {
  const manifest = candidate.packageGimli;
...
function buildExternalCatalogEntry(entry: ExternalCatalogEntry): ChannelPluginCatalogEntry | null {
  const manifest = entry.gimli ?? entry[LEGACY_MANIFEST_KEY];
  return buildCatalogEntry({
    packageName: entry.name,
    packageGimli: manifest,
  });
}
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverGimliPlugins({ workspaceDir: options.workspaceDir });
