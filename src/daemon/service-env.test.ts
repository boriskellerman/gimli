...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.GIMLI_GATEWAY_PORT).toBe("18789");
    expect(env.GIMLI_GATEWAY_TOKEN).toBe("secret");
    expect(env.GIMLI_SERVICE_MARKER).toBe("gimli");
    expect(env.GIMLI_SERVICE_KIND).toBe("gateway");
    expect(typeof env.GIMLI_SERVICE_VERSION).toBe("string");
    expect(env.GIMLI_SYSTEMD_UNIT).toBe("gimli-gateway.service");
    if (process.platform === "darwin") {
      expect(env.GIMLI_LAUNCHD_LABEL).toBe("bot.molt.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", GIMLI_PROFILE: "work" },
      port: 18789,
    });
    expect(env.GIMLI_SYSTEMD_UNIT).toBe("gimli-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.GIMLI_LAUNCHD_LABEL).toBe("bot.molt.work");
    }
  });
});
