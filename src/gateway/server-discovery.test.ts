...
describe("resolveTailnetDnsHint", () => {
  ...
  beforeEach(() => {
    prevTailnetDns.value = process.env.GIMLI_TAILNET_DNS;
    delete process.env.GIMLI_TAILNET_DNS;
  ...
  afterEach(() => {
    if (prevTailnetDns.value === undefined) {
      ...
      process.env.GIMLI_TAILNET_DNS = prevTailnetDns.value;
    }
  });

  test("returns env hint when disabled", async () => {
    process.env.GIMLI_TAILNET_DNS = "studio.tailnet.ts.net.";
