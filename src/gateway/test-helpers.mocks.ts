...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `gimli-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.GIMLI_CONFIG_PATH = path.join(root, "gimli.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "gimli.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "gimli-gateway-test"),
        ...fileDefaults,
