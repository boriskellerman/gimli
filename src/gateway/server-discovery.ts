...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "Gimli";
  if (/gimli/i.test(trimmed)) return trimmed;
  return `${trimmed} (Gimli)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.GIMLI_CLI_PATH?.trim();
  ...
  const siblingCli = path.join(execDir, "gimli");
  ...
  const binCli = path.join(cwd, "bin", "gimli.js");
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.GIMLI_TAILNET_DNS?.trim();
