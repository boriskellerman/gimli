...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-gimli-message-channel") ?? "");
  const accountId = getHeader(req, "x-gimli-account-id")?.trim() || undefined;
  ...
  const allTools = createGimliTools({
