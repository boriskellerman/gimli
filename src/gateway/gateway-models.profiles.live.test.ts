...
import { resolveGimliAgentDir } from "../agents/agent-paths.js";
...
import { ensureGimliModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { GimliConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.GIMLI_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.GIMLI_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.GIMLI_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<GimliConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.GIMLI_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.GIMLI_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveGimliAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "gimli-live-state-"));
  ...
  process.env.GIMLI_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.gimli-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveGimliAgentDir();
  const sanitizedCfg: GimliConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Gimli live tool probe (local, safe): " +
    ...
    process.env.GIMLI_CONFIG_PATH = previous.configPath;
    process.env.GIMLI_GATEWAY_TOKEN = previous.token;
    process.env.GIMLI_SKIP_CHANNELS = previous.skipChannels;
    process.env.GIMLI_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.GIMLI_SKIP_CRON = previous.skipCron;
    process.env.GIMLI_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.GIMLI_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.GIMLI_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureGimliModelsJson(cfg);
      ...
      const rawModels = process.env.GIMLI_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.gimli-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
