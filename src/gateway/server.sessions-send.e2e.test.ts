...
import { createGimliTools } from "../agents/gimli-tools.js";
...
beforeAll(async () => {
  ...
  prevGatewayToken = process.env.GIMLI_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.GIMLI_GATEWAY_PORT = String(gatewayPort);
  process.env.GIMLI_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.GIMLI_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createGimliTools().find((candidate) => candidate.name === "sessions_send");
