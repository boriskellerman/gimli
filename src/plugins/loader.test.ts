...
import { loadGimliPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.GIMLI_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `gimli-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "gimli.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.GIMLI_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadGimliPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.GIMLI_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadGimliPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadGimliPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
