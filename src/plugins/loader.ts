...
import { discoverGimliPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: GimliPluginDefinition;
  register?: GimliPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as GimliPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as GimliPluginDefinition;
...
export function loadGimliPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverGimliPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: {
            "gimli/plugin-sdk": pluginSdkAlias,
          },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: GimliPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as GimliPluginModule;
    } catch (err) {
