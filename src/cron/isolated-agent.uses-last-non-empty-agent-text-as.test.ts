...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "gimli-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".gimli", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<GimliConfig> = {},
): GimliConfig {
  const base: GimliConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "gimli"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".gimli", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
