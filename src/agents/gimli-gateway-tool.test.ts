...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const previousProfile = process.env.GIMLI_PROFILE;
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "gimli-test-"));
    ...
    try {
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: gimli --profile isolated doctor --non-interactive",
      );
