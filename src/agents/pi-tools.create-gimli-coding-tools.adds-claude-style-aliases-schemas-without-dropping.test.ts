...
import { __testing, createGimliCodingTools } from "./pi-tools.js";
...
describe("createGimliCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createGimliCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "gimli-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "gimli-outside.txt");
