...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "gimli.sandbox=1");
  args.push("--label", `gimli.sessionKey=${params.scopeKey}`);
  args.push("--label", `gimli.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `gimli.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "gimli.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`gimli sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`gimli sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("gimli sandbox recreate --all");
}
