...
export function formatAssistantErrorText(
  ...
  opts?: { cfg?: GimliConfig; sessionKey?: string },
): string | undefined {
...
export function sanitizeUserFacingText(text: string): string {
  ...
  // Only apply error-prefix rewriting to short texts that look like raw API error
  // responses, not conversational text that happens to start with "Error" or "Failed".
  // Fixes #3594: user messages containing error-pattern phrases were being rewritten.
  if (ERROR_PREFIX_RE.test(trimmed) && trimmed.length < 500) {
