...
/**
 * Cleanup suspended Gimli CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: GimliConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "gimli-cli-images-"));
  const paths: string[] = [];
