...
describe("auth-profiles (chutes)", () => {
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.GIMLI_AGENT_DIR;
    else process.env.GIMLI_AGENT_DIR = previousAgentDir;
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "gimli-chutes-"));
    process.env.GIMLI_STATE_DIR = tempDir;
    process.env.GIMLI_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");
