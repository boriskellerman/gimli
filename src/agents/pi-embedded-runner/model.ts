...
// pi-ai's built-in Anthropic catalog can lag behind Gimli's defaults/docs.
...
function resolveOpenAICodexGpt53FallbackModel(
  ...
  if (normalizedProvider !== "openai-codex") return undefined;
  if (trimmedModelId.toLowerCase() !== OPENAI_CODEX_GPT_53_MODEL_ID) return undefined;
...
function resolveAnthropicOpus46ForwardCompatModel(
  ...
  const normalizedProvider = normalizeProviderId(provider);
  if (normalizedProvider !== "anthropic") return undefined;
...
export function buildModelAliasLines(cfg?: GimliConfig) {
...
export function resolveModel(
  ...
  const resolvedAgentDir = agentDir ?? resolveGimliAgentDir();
