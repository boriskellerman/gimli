...
export async function runEmbeddedPiAgent(
  ...
  return enqueueSession(() =>
    enqueueGlobal(async () => {
      ...
      const agentDir = params.agentDir ?? resolveGimliAgentDir();
      const fallbackConfigured =
        (params.config?.agents?.defaults?.model?.fallbacks?.length ?? 0) > 0;
      await ensureGimliModelsJson(params.config, agentDir);
          ...
          if (promptError && !aborted) {
            ...
            if (isContextOverflowError(errorText)) {
              ...
              if (!isCompactionFailure && !overflowCompactionAttempted) {
                ...
                // Fix #3560: flush memories BEFORE truncating conversation context
                if (params.onBeforeCompact) {
                  try {
                    await params.onBeforeCompact();
                  } catch (err) {
                    log.warn(`pre-compaction memory flush failed: ${describeUnknownError(err)}`);
                  }
                }
