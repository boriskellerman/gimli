...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "gimli-models-" });
}

const MODELS_CONFIG: GimliConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureGimliModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureGimliModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureGimliModelsJson } = await import("./models-config.js");
      const { resolveGimliAgentDir } = await import("./agent-paths.js");

      await ensureGimliModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveGimliAgentDir(), "models.json");
