...
export function listChannelSupportedActions(params: {
  ...
  const cfg = params.cfg ?? ({} as GimliConfig);
...
export function listChannelAgentTools(params: { cfg?: GimliConfig }): ChannelAgentTool[] {
...
function runPluginListActions(plugin: ChannelPlugin, cfg: GimliConfig): ChannelMessageActionName[] {
  if (!plugin.actions?.listActions) return [];
  try {
