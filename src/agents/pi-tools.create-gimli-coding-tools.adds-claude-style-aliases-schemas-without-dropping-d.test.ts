...
const defaultTools = createGimliCodingTools();

describe("createGimliCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "gimli-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createGimliCodingTools();
    ...
    try {
      ...
      const contents = "Hello from gimli read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "gimli-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "gimli-workspace"),
    ...
    const tools = createGimliCodingTools({ sandbox });
