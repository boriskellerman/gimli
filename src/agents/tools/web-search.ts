...
type WebSearchConfig = NonNullable<GimliConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: GimliConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("gimli configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.gimli.bot/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://gimli.bot",
      "X-Title": "Gimli Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
