...
function resolveDefaultModelRef(cfg?: GimliConfig): {
  provider: string;
  model: string;
} {
  if (cfg) {
...
export function resolveImageModelConfigForTool(params: {
  ...
  if (preferred?.trim()) {
    ...
    if (anthropicOk) addFallback("anthropic/claude-opus-4-6");
...
function pickMaxBytes(cfg?: GimliConfig, maxBytesMb?: number): number | undefined {
...
async function runImagePrompt(params: {
  ...
  const effectiveCfg: GimliConfig | undefined = params.cfg
    ? {
        ...params.cfg,
        agents: {
          ...params.cfg.agents,
  ...
  await ensureGimliModelsJson(effectiveCfg, params.agentDir);
