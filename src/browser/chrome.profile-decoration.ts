...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".gimli-profile-decorated");
}
 ...
 */
export function decorateGimliProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_GIMLI_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_GIMLI_BROWSER_COLOR).toUpperCase();
